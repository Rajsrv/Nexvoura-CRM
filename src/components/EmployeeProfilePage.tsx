  ...
  return (
    <div className="p-4 md:p-8 space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm">Managing details for {employee.name}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200 shadow-sm">
            <button 
              ...
              className="p-2.5 hover:bg-white text-slate-400 hover:text-emerald-600 rounded-xl transition-all group"
            ...
            <button 
              ...
              className="p-2.5 hover:bg-white text-slate-400 hover:text-rose-600 rounded-xl transition-all group"
          ...
          <button
            ...
            className={`px-6 py-2.5 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all flex items-center space-x-2 ${
              activeTab === 'profile' ? 'bg-slate-950 text-white shadow-xl shadow-slate-200' : 'bg-white text-slate-400 border border-slate-100 hover:bg-slate-50'
            }`}
          >
            <User size={14} />
      ...
      <div className="bg-white rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden">
        <div className="p-6 md:p-8 bg-slate-50 border-b border-slate-200/60">
          <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-8">
            <div className="relative group">
              <div className="w-24 h-24 md:w-32 md:h-32 rounded-3xl bg-brand-primary/10 flex items-center justify-center overflow-hidden border-4 border-white shadow-xl">
                {employee.photoURL ? (
              ...
              {canManage && (
                <button className="absolute -bottom-2 -right-2 p-2 bg-white rounded-xl border border-slate-200 shadow-lg text-slate-500 hover:text-brand-primary transition-colors">
                  <Upload size={16} />
                </button>
              )}
            </div>
            <div className="text-center md:text-left flex-1">
              ...
              <p className="text-brand-primary font-bold text-sm tracking-widest uppercase mt-1">
        ...
        {/* Tabs */}
        <div className="flex overflow-x-auto border-b border-slate-200/60 px-4 pt-4 bg-white scrollbar-hide">
          ...
          <AnimatePresence mode="wait">
              ...
              >
                <div className="space-y-6">
                  <h3 className="font-bold text-slate-900 flex items-center">
                    <Mail size={18} className="mr-2 text-brand-primary" /> Contact Information
                  </h3>
                ...
                <div className="space-y-6">
                  <h3 className="font-bold text-slate-900 flex items-center">
                    <User size={18} className="mr-2 text-brand-primary" /> Emergency Contact
                  </h3>
              ...
              >
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <h3 className="font-bold text-slate-900 flex items-center">
                      <CreditCard size={18} className="mr-2 text-brand-primary" /> Bank Details
                    </h3>
                    {canManage && (
                      <button 
                  ...
                  <div className="space-y-4 p-4 bg-slate-50 rounded-2xl border border-slate-200/60 relative">
                    {!isOwnProfile && !canManage && (
                      <div className="absolute inset-0 bg-slate-50/80 backdrop-blur-[2px] flex flex-col items-center justify-center rounded-2xl z-10">
                        <Lock size={24} className="text-slate-400 mb-2" />
                        <p className="text-xs font-bold text-slate-500">Restricted Access</p>
                      </div>
                    )}
                ...
                <div className="space-y-6">
                  <h3 className="font-bold text-slate-900 flex items-center">
                    <ShieldCheck size={18} className="mr-2 text-brand-primary" /> Identification
                  </h3>
                  <div className="space-y-4 p-4 bg-slate-50 rounded-2xl border border-slate-200/60">
            ...
            {activeTab === 'salary' && canSeeSensitive && (
              ...
              >
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-slate-900 flex items-center">
                    <TrendingUp size={18} className="mr-2 text-brand-primary" /> Salary History & Proposals
                  </h3>
                  {canManage && (
                    <button 
                      onClick={() => setShowHikeModal(true)}
                      className="flex items-center space-x-2 text-sm font-bold bg-slate-950 text-white px-4 py-2 rounded-xl hover:bg-brand-primary transition-all shadow-lg shadow-slate-200"
                    >
                      <Plus size={16} />
                ...
                <div className="border border-slate-200/60 rounded-2xl overflow-hidden shadow-sm">
                  <table className="w-full text-left">
                    <thead className="bg-slate-50 border-b border-slate-200/60 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                      <tr>
                        ...
                        <th className="px-6 py-4 text-brand-primary">Projected</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {(employee.salaryHistory || []).length > 0 ? (
                        employee.salaryHistory?.map((hike) => (
                          <tr key={hike.id} className="hover:bg-slate-50 transition-colors group">
                            <td className="px-6 py-4">
                              <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded inline-block ${
                                ...
                                hike.status === 'rejected' ? 'bg-rose-100 text-rose-700' : 
                            ...
                            <td className="px-6 py-4 text-sm text-slate-600 max-w-xs">{hike.reason}</td>
                            <td className="px-6 py-4 text-sm text-slate-400 line-through">
            ...
            {activeTab === 'docs' && (
              ...
              >
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-slate-900 flex items-center">
                    <FileText size={18} className="mr-2 text-brand-primary" /> Employee Documents
                  </h3>
                  {canSeeSensitive && (
                    <label className="flex items-center space-x-2 text-sm font-bold bg-brand-primary text-white px-4 py-2 rounded-xl hover:shadow-lg hover:scale-[1.02] cursor-pointer transition-all active:scale-[0.98]">
                      <Upload size={16} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {documents.length > 0 ? (
                    documents.map((doc) => (
                      <div key={doc.id} className="p-4 bg-white border border-slate-200/60 rounded-2xl flex items-center justify-between hover:shadow-md transition-all group">
                        ...
                        <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button className="p-2 text-slate-400 hover:text-brand-primary rounded-lg hover:bg-brand-primary/5">
                            <Download size={16} />
                          </button>
                          {canManage && (
                            <button className="p-2 text-slate-400 hover:text-rose-500 rounded-lg hover:bg-rose-50">
                              <Trash2 size={16} />
                            </button>
                          )}
                        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <div>
                  <h2 className="text-2xl font-black text-slate-900">Edit Sensitive Data</h2>
                  <p className="text-slate-500 text-sm">Update banking and identification records</p>
                </div>
                <button onClick={() => setShowEditSensitive(false)} className="p-2 hover:bg-slate-100 rounded-xl transition-colors">
              ...
              <div className="p-8 max-h-[60vh] overflow-y-auto custom-scrollbar space-y-8">
                ...
                <div className="space-y-4">
                  <p className="text-[10px] font-bold text-brand-primary uppercase tracking-[0.2em] mb-4">Bank Information</p>
                ...
                {/* Gov ID Section */}
                <div className="space-y-4">
                  <p className="text-[10px] font-bold text-brand-primary uppercase tracking-[0.2em] mb-4">Identification</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">ID Type</label>
                      <select 
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">ID Number</label>
                      <input
                        type="text"
                        className="saas-input"
      ...
      <AnimatePresence>
        {showHikeModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="mb-8">
                <h3 className="text-3xl font-black text-slate-950 font-display italic leading-none mb-2">Propose Adjustment</h3>
                <p className="text-slate-500 text-sm">Compensation vectors for {employee?.name}</p>
              </div>

              <div className="space-y-6">
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 px-1">Increment Amount ({company?.currency || '$'})</label>
                  <input
                    type="number"
                    className="w-full p-5 bg-slate-50 border border-slate-100 rounded-3xl outline-none focus:ring-4 focus:ring-brand-primary/10 transition-all font-black"
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 px-1">Justification Reason</label>
                  <textarea
                    className="w-full p-5 bg-slate-50 border border-slate-100 rounded-3xl outline-none focus:ring-4 focus:ring-brand-primary/10 transition-all text-sm font-medium h-32 resize-none"
                ...
                <div className="flex bg-blue-50 p-6 rounded-3xl border border-blue-100 items-center space-x-4">
                  <div className="p-3 bg-blue-500 rounded-2xl text-white">
                    <TrendingUp size={24} />
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1">New Projected Salary</p>
                ...
                <div className="grid grid-cols-2 gap-4 pt-4">
                  <button
                    onClick={() => setShowHikeModal(false)}
                    className="py-5 rounded-3xl font-black text-xs uppercase tracking-widest text-slate-500 hover:text-slate-950 transition-all"
                  >
                    Abort
                  </button>
                  <button
                    onClick={handleAddHike}
                    className="bg-slate-950 text-white py-5 rounded-3xl font-black text-xs uppercase tracking-widest hover:bg-brand-primary transition-all shadow-xl shadow-slate-200"
                  >
                    Submit Proposal
                  </button>
                </div>
