...
export function PermissionsPage({ user }: { user: UserProfile }) {
  ...
  return (
    <div className="p-4 md:p-8 pt-6 max-w-7xl mx-auto space-y-10 min-h-screen bg-white">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-100">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-black tracking-tight text-slate-950 font-display italic leading-none">Access Control</h1>
          <p className="text-slate-500 font-medium">Manage hierarchical permissions and operative clearance</p>
        </div>
        
        {!isAdmin && (
          <button 
            onClick={() => setShowRequestModal(true)}
            className="saas-button-primary px-6 py-3 shadow-xl shadow-indigo-500/20 flex items-center space-x-2 group"
          >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
        ...
        <div className="xl:col-span-2 space-y-6">
          <div className="bg-[#fcfcfc] border border-slate-100 rounded-[32px] p-6 shadow-sm overflow-hidden">
             <div className="flex items-center justify-between mb-6">
                <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400">Personnel Directory</h3>
                <div className="relative w-64">
                   ...
                   <input 
                     ...
                     className="w-full pl-9 pr-4 py-2 bg-white border border-slate-100 rounded-xl text-xs font-bold outline-none focus:ring-4 focus:ring-indigo-500/5 transition-all"
                   />
                </div>
             </div>
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black ${
                        selectedUser?.uid === u.uid ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400'
                      }`}>
                        {u.name[0]}
                      </div>
                      <div className="text-left">
                         <p className="text-sm font-bold truncate max-w-[150px]">{u.name}</p>
                         <p className={`text-[10px] uppercase font-black tracking-widest ${selectedUser?.uid === u.uid ? 'text-indigo-300/80' : 'text-slate-400'}`}>
                           {u.role}
                         </p>
                      </div>
                    </div>
                    <ChevronRight size={16} className={selectedUser?.uid === u.uid ? 'text-indigo-400' : 'text-slate-200'} />
                  </button>
                ))}
             </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            {selectedUser ? (
              <motion.div
                ...
                exit={{ opacity: 0 }}
                className="bg-white border border-slate-100 rounded-[40px] p-8 shadow-2xl relative overflow-hidden"
              >
                <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-50/50 rounded-bl-full -z-10" />
                
                <div className="flex justify-between items-start mb-10">
                   <div>
                      <h2 className="text-2xl font-black text-slate-950 mb-1">{selectedUser.name}</h2>
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{selectedUser.role} clearance profile</p>
                   </div>
                   <div className="flex items-center space-x-2 bg-slate-950 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   {AVAILABLE_PERMISSIONS.map(perm => {
                     ...
                     return (
                       ...
                       >
                          <div className="flex items-center space-x-4">
                             <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${isGranted ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-200 text-slate-400'}`}>
                                {isGranted ? <Shield size={18} /> : <Lock size={18} />}
                             </div>
                             <div>
                                <p className="text-xs font-black uppercase tracking-widest text-slate-950">{perm.label}</p>
                                <p className="text-[10px] text-slate-500 font-medium">{perm.description}</p>
                             </div>
                          </div>
                          {isGranted && <Check size={20} className="text-indigo-600" />}
                       </div>
                     );
                   })}
                </div>
              ...
              <div className="py-20 text-center border-2 border-dashed border-slate-100 rounded-[40px] bg-slate-50/30">
                 ...
                 <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">Select personnel to manage clearance</p>
              </div>
            )}
          </AnimatePresence>
        </div>
        ...
        <div className="space-y-6">
           ...
           <div className="bg-white border border-slate-100 rounded-[32px] p-6 shadow-sm">
              <div className="flex items-center space-x-3 mb-6">
                 ...
                 <h3 className="text-xs font-black uppercase tracking-widest text-slate-400">Hierarchy Guide</h3>
              </div>
              <div className="space-y-4">
                 <div className="flex items-start space-x-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-1.5" />
                    <p className="text-[11px] text-slate-500 leading-relaxed"><span className="font-bold text-slate-700 uppercase">Admins</span> have total operative authority over all system nodes.</p>
                 </div>
                 <div className="flex items-start space-x-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-1.5" />
                    <p className="text-[11px] text-slate-500 leading-relaxed"><span className="font-bold text-slate-700 uppercase">Managers</span> can override clearance for Sales and Team Leads.</p>
                 </div>
                 <div className="flex items-start space-x-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-1.5" />
                    <p className="text-[11px] text-slate-500 leading-relaxed"><span className="font-bold text-slate-700 uppercase">Requests</span> must be approved by a superior operative for activation.</p>
                 </div>
              </div>
           </div>
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md">
            ...
            >
              ...
              <div className="flex items-center space-x-3 mb-6">
                 ...
                 <div>
                    <h3 className="text-2xl font-black text-slate-950 font-display italic leading-none">Security Request</h3>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Escalation of Clearance</p>
                 </div>
              </div>

              <div className="space-y-6">
                 <div>
                   <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 px-1">Requested Module</label>
                   <select 
                     className="saas-input appearance-none bg-slate-50"
                 ...
                 <div>
                   <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 px-1">Justification</label>
                   <textarea 
                     className="saas-input min-h-[120px] bg-slate-50 resize-none py-4"
                 ...
                 <button 
                   ...
                   className="w-full saas-button-primary py-4 shadow-2xl shadow-indigo-200"
                 >
